...
class PredictorLookahead:
    ...
    def __init__(self, lookahead, n_features, features_name, prediction_type, add_pkt_indices, conditioning=False, granularity=None,embedding=False):
        self.lookahead = lookahead
        self.n_features = n_features
        self.features_name = features_name
        self.prediction_type = prediction_type if prediction_type in prediction_type_list else 'recursive'
        self.add_pkt_indices = add_pkt_indices
    ...
    def predictor_nn(self, nn_models, histories, labels, fit_next_model=False):
        ...
        if (self.prediction_type == 'hybrid' or self.prediction_type == 'ouroboros') and fit_next_model:
            prediction_steps = 1
        predicted_dict = dict((k, v) for (k, v) in zip(self.features_name, [np.zeros(shape=(histories[1].shape[0] if self.embedding else histories.shape[0],prediction_steps)) for _ in range(len(self.features_name))]))

        for i in range(len(nn_models)):
            model = nn_models[i]
            if (self.conditioning):
                predicted = model.predict([histories[1],histories[0], labels]) if self.embedding else model.predict([histories, labels])
            else:
                predicted = model.predict([histories[1],histories[0]]) if self.embedding else model.predict(histories)
                
            for j, key in enumerate(predicted_dict.keys()):
                predicted_dict[key][:, i] = predicted[j][:, 0]

            if self.prediction_type == 'hybrid':
                num_elem = histories[1].shape[0] if self.embedding else histories.shape[0]
                win_size = histories[1].shape[1] + 1 if self.embedding else histories.shape[1] + 1
